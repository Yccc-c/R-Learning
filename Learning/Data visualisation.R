...
ggplot(data = smaller) +  # 像素风格
  geom_bin2d(mapping = aes(x = carat, y = price))

# install.packages("hexbin")
ggplot(data = smaller) +  # 六边形
...
ggplot(data = diamonds) +
  geom_histogram(mapping = aes(x = carat), binwidth = 0.5) # 直方图
ggplot(data = diamonds, mapping = aes(x = carat, colour = cut)) +
  geom_freqpoly(binwidth = 0.5) # 和上图类似，不过用线条表达，便于覆盖
ggplot(data = diamonds, mapping = aes(x = cut, y = price)) +
  geom_boxplot()  # 箱线图
  ggplot(data = mpg) + #多重箱线图排序
    geom_boxplot(mapping = aes(x = reorder(class, hwy, FUN = median), y = hwy)) +
    coord_flip()
  ggplot(data = diamonds) + # 点阵图
    geom_count(mapping = aes(x = cut, y = color))
  diamonds %>% 
    count(color, cut) %>%  
    ggplot(mapping = aes(x = color, y = cut)) + # 颜色矩形图
      geom_tile(mapping = aes(fill = n)) # R4ds 7.5.2
